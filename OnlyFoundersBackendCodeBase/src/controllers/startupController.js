...
exports.submitTractionDetails = async (req, res) => {
  try {
    ...
    const startup = await Startup.findOneAndUpdate(
        { user_id: userId }, // Correct query filter
        {
          $set: {
            ...
            "traction.githubStars": githubStars,
            "traction.storageCapacity": storageCapacity,
            "traction.nodeOperators": nodeOperators,
            // "traction.developerInterest": developerInterest,
    ...
    res.status(200).json({ message: "Step 2 updated successfully", startup });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.addTeamMember = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Team member added successfully.", startup });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.submitStartupRoadmap = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "Roadmap updated successfully", startup });
...
exports.submitStartupTokenomics = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: "Tokenomics updated successfully, Startup marked as completed", startup });
...
exports.viewStartup = async(req,res)=>{
    ...
    try{
        ...
        return res.status(200).json({message:"Startup detail",startup});
