...
exports.isWalletConnected = async(req,res)=>{
  try{
    ...
    if (user.walletAddress=== "" || !user.walletAddress) {
      return res.status(200).json({ message: "Wallet not connected" });
    }
    else{
      return res.status(200).json({ message: "Wallet connected" });
    }
...
// get investor details in founder dashboard

exports.getInvestorDetails = async (req, res) => {
  try {
    const userId=req.user_id; // Assuming you get the profileId from the request body

    // Fetch the startup project
    
    const campaign_id=await Campaign.findOne({user_id:userId}).lean().select("_id");
    if (!campaign_id) {
